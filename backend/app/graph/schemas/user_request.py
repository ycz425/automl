...
class UserRequest(ClarifiableModel):
    ...
    primary_metric: MetricEntry | None = Field(default=None, description="Main metric used to compare experiments, select checkpoints, and decide whether an experiment improved. Return null if it cannot be determined.")
    secondary_metrics: list[MetricEntry] = Field(min_length=1, description="Metrics to compute when evaluating the model, such as F1, AUROC, accuracy, RMSE, MAE, or R-squared. Must not include the primary metric. Select an appropriate set of metrics for the task if not provided by user.")
